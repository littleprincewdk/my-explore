...
function addPhotos(){
    ...
    for(var i = 0; i < data.length; i++){//for in 循环中的循环计数器是字符串，而不是数字它包含当前属性的名称或当前数组元素的索引
        _wrap += '<div class="photo  photo_front" onclick="turn(this)" id="photo_' + (i + 1) + '"><div class="photo-wrap"><div class="side side-front"><p class="image"><img src="Plugin/my/gallery/images/' + data[i].img + '"></p><p class="caption">' + data[i].caption + '</p></div><div class="side side-back"><p class="desc">' + data[i].desc + '</p></div></div></div></div>';
...
function range(){
    ...
    range.left.y = [0 + photo.height/4 , wrap.height - photo.height/ 4];
...
for(var i=0;i<g(".photo").length;i++){
    g(".photo")[i].style.width=settings.photo_width+"px";
    g(".photo")[i].style.height=settings.photo_height+"px";
    g(".photo")[i].style.margin=-settings.photo_height/2+"px 0 0 "+(-settings.photo_width/2)+"px";
}
